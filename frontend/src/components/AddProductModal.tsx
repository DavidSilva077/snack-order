...
export default function AddProductModal({ open, onClose, onSubmit }: Props) {
  const [form, setForm] = React.useState({ nome: '', preco: '', categoria: '' })
  ...
  const canSave = !!form.nome && !!form.categoria && precoNumber > 0
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center">
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="relative w-full max-w-md rounded-lg bg-white p-4 shadow"
      >
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <button
            onClick={async () => {
              await onSubmit({ ...form, preco: precoNumber })
              onClose()
            }}
